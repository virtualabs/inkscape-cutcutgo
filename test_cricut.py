...
import sys, os
from silhouette.Cutcutgo import CricutMaker, CRICUT_MATS
...
dev = CricutMaker(progress_cb=write_progress, dry_run=False)
state = dev.status()    # hint at loading paper, if not ready.
print("status=%s" % (state))
print("device version: '%s'" % dev.get_version())

dev.setup(media=132, pen=False, pressure=1, speed=10, toolholder=1)
